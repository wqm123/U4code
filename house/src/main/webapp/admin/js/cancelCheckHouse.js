...
function ModifyBySelect(){
    ...
    if (SelectRows.length==1){
        ...
        //2通过传入id获取数据
        $.post("findDistrictById",{"id":SelectRows[0].id},function (data) {
...
function singleCheck(id) {
            ...
            $.post("CancelCheckHouse",{"id":id},function (data) {
                if (data.result==1){
                    $("#dg").datagrid("reload")
                    //CloseDialog("AddDialog")
                } else{
        ...
        $.messager.confirm('确认提示','真的要删除这'+s.length+'条数据吗?',function(r){
            if (r) {
                ...
                for (var i = 0; i < s.length; i++) {
                    $.post("delDistrict",{"id":s[i].id},function (data) {
                        var d = data.result;
                        if (data.result==1){
                            flag++;
                           while (flag==s.length){
        ...
        $.messager.confirm('确认提示','真的要删除这'+s.length+'条数据吗?',function(r){
            if (r) {
                ...
                * 如果你想要用传统的方式来序列化数据，那么就设置为 true。*/
                ...
                $.post("delMoreDistricts" ,{"ids": ids}, function (data) {
