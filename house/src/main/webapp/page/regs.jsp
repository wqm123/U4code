...
<DIV class=box>
<FORM action="addUsers" method="post" id="form1" >
<DIV class=infos>
<TABLE class=field>
  <TBODY>
  <TR>
    <TD class=field>用 户 名：</TD>
    <TD><INPUT class=text type=text name=name onchange="ch()" id="username"> <span id="si"></span></TD></TR>
...
<DIV id=footer class=wrap>
  <script>
    function ch() {
        $.post("/page/confirmName",{"name":$("#username").val()},
            function(data){
            if (data.result==1){
                $("#si").text("用户名已存在").css("color","red");
                $("#form1").submit(false)
